...
The goal here is transform a computation into polynomials that the prover can use to develop a proof that the computation was done correctly. If you do not know what a polynomial is, it is simply an equation represented in the form f(x) = c<sub>3</sub>x<sup>n</sup>+…+c<sub>1</sub>x<sup>1</sub>+c<sup>0</sub>x<sup>0</sup> where all c’s are constants and x in the input. The highest power of x determines the degree of the polynomial. For instance, 2x<sup>4</sup>-1x<sup>3</sup>+0x<sup>2</sup>+0x<sup>1</sup>+12x<sup>0</sup> is a polynomial of degree four, and it can be rewritten as 2x<sup>4</sup>-1x<sup>3</sup>+12. The reason why polynomials are needed will be explained later - just know that this is an essential step. 
