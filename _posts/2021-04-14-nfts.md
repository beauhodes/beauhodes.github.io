...
When creating this NFT, Beeple used a token (which, as we discussed, is really just sitting on a smart contract) called MakersTokenV2. To give you a better look at how it works, one example of an ERC-721 required function is tokenURI which returns a stored link for an NFT that is requested by its token number. The code for this function in MakersTokenV2 looks like this:
