...
Interestingly enough, the Beeple piece we looked at earlier has this problem. That IPFS link above actually just takes us to more metadata (which, on the positive side, is stored on IPFS) that contains the actual link to the image. Guess what the image’s link begins with?
